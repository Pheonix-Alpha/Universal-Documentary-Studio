...
# ================================================================
# GENERATE BASICSR VERSION FILE
# ================================================================
#
# BasicSR normally creates basicsr/version.py during installation.
# We are intentionally NOT installing BasicSR because its legacy
# setup.py is problematic on Python 3.13.
#
# Therefore we generate the small runtime version module ourselves.
#

echo ""
echo "Preparing BasicSR version metadata..."

BASICSR_VERSION_FILE="$BASICSR_DIR/basicsr/version.py"

if [ -f "$BASICSR_VERSION_FILE" ]; then

    echo "BasicSR version.py already exists."

else

    BASICSR_VERSION=$(cat "$BASICSR_DIR/VERSION" 2>/dev/null || echo "unknown")
    BASICSR_GITSHA=$(git -C "$BASICSR_DIR" rev-parse HEAD 2>/dev/null || echo "unknown")

    cat > "$BASICSR_VERSION_FILE" <<EOF
# Generated by Universal Documentary Studio setup
__version__ = "${BASICSR_VERSION}"
__gitsha__ = "${BASICSR_GITSHA}"
version_info = ()
EOF

    echo "BasicSR version.py generated."
...
# ================================================================
# GENERATE REAL-ESRGAN VERSION FILE
# ================================================================

echo ""
echo "Preparing Real-ESRGAN version metadata..."

REALESRGAN_VERSION_FILE="$REALESRGAN_DIR/realesrgan/version.py"

if [ -f "$REALESRGAN_VERSION_FILE" ]; then

    echo "Real-ESRGAN version.py already exists."

else

    REALESRGAN_VERSION=$(cat "$REALESRGAN_DIR/VERSION" 2>/dev/null || echo "unknown")
    REALESRGAN_GITSHA=$(git -C "$REALESRGAN_DIR" rev-parse HEAD 2>/dev/null || echo "unknown")

    cat > "$REALESRGAN_VERSION_FILE" <<EOF
# Generated by Universal Documentary Studio setup
__version__ = "${REALESRGAN_VERSION}"
__gitsha__ = "${REALESRGAN_GITSHA}"
version_info = ()
EOF

    echo "Real-ESRGAN version.py generated."
...
print("Real-ESRGAN version:", realesrgan.__version__)
...
print("BasicSR  : OK")
print("Version  :", basicsr.__version__)
...
print("ESRGAN   : OK")
print("Version  :", realesrgan.__version__)
