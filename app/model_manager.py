...
MODEL_REGISTRY = {
    ...
    "SG161222/Realistic_Vision_V5.1_noVAE": {
        ...
        "type": "image",
        "size_gb": 4.8,
        "description": "Text-to-image generation for documentary scene images",
        "vram_gb": 6.0,
    },
...
def is_installed(model_id: str) -> bool:
    ...
    for root, dirs, files in os.walk(model_path):
        if any(
            f in required_files or f.endswith(".bin") or f.endswith(".safetensors")
            for f in files
        ):
            has_files = True
            break
...
def auto_download_and_load_model(
    model_id: str, device: str = None, progress_callback=None
) -> Any:
    """
...
def smart_download_model(
    model_id: str, force: bool = False, progress_callback=None
    ...
    try:
        ...
        if model_size > available:
            ...
            if freed < needed:
                yield (
                    50,
                    f"❌ Cannot free enough space. Freed {freed:.2f}GB, needed {needed:.2f}GB",
                )
...
def _load_model_into_vram(model_id: str, device: str) -> Any:
    ...
    if model_info["type"] == "video":
        ...
        return _load_image_model(model_id, device)
...
def _load_image_model(model_id: str, device: str) -> Dict[str, Any]:
...
def generate_text(
    ...
    loaded = auto_download_and_load_model(model_id, progress_callback=progress_callback)
    ...
    try:
        ...
        for text_chunk in streamer:
            ...
            chunk_tokens = len(tokenizer.encode(text_chunk, add_special_tokens=False))
            ...
            if now - last_update_time >= 0.5 or generated_tokens == 1:

                percentage = min(99, int(generated_tokens / max_new_tokens * 100))

                tokens_per_second = generated_tokens / elapsed if elapsed > 0 else 0

                remaining_tokens = max(0, max_new_tokens - generated_tokens)

                eta_seconds = (
                    remaining_tokens / tokens_per_second if tokens_per_second > 0 else 0
                )
