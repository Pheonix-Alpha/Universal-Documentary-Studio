...
def _generate_scene_image(
    ...
    """
    Generate a real visual scene image using the image model.
    The resulting image will be used as input to Stable Video Diffusion.
    """

    print("🖼️ Generating real scene image...")
    print(f"   Image model: {IMAGE_MODEL_ID}")

    # Free VRAM from any currently loaded model.
    model_manager._unload_unused_models(keep=[])

    loaded = model_manager.auto_download_and_load_model(IMAGE_MODEL_ID)
    ...
    # SD 1.5-style models normally work best at 512 resolution.
    image = pipe(
        prompt=prompt,
        height=512,
        width=512,
    ...
    print("✅ Real scene image generated")

    # The image model is no longer needed.
    # SVD needs the VRAM.
    model_manager._unload_unused_models(keep=[])
    ...
    try:
        ...
        if reference_image:
            ...
            if reference_image.startswith("data:image"):
                img_data = reference_image.split(",", 1)[1]
                img = Image.open(BytesIO(base64.b64decode(img_data))).convert("RGB")
            else:
                img = load_image(reference_image).convert("RGB")
                print("🖼️ Using provided reference image")

        else:
            print("🖼️ No reference image provided")
            print("🧠 Creating scene image from prompt...")
            img = _generate_scene_image(
...
def _generate_fallback_video(
    prompt: str, duration: int, fps: int, width: int, height: int, seed: int
) -> Dict[str, Any]:
...
def _get_model_path(model_id: str) -> str:
    ...
    try:
        ...
        return os.path.join(
            os.path.expanduser("~"), ".cache", "models", model_id.replace("/", "__")
        )
...
def _frames_to_video_bytes(frames: List[np.ndarray], fps: int) -> bytes:
    ...
    try:
        ...
        for frame in frames:
            ...
            if isinstance(frame, Image.Image):
                ...
                if len(frame.shape) == 2:
                    ...
                    frame = Image.fromarray(frame, mode="L").convert("RGB")

                elif frame.shape[2] == 4:

                    frame = Image.fromarray(frame, mode="RGBA").convert("RGB")
        ...
        print(f"🎞️ Preparing {len(normalized_frames)} frames...")
        ...
        if not os.path.exists(video_path):
            ...
            print("❌ FFmpeg did not create video file")
        ...
        print(f"✅ H.264 MP4 created: " f"{len(video_bytes) / 1024:.1f} KB")

        return video_bytes

    except Exception as e:

        print(f"❌ Failed to convert frames " f"to video: {e}")
